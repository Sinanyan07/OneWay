...
{ //header opening language
  ...
  const dropDw = document.querySelector('.dropDw');
  const services = document.querySelector('.services');
  const serv = document.getElementById('serv');
  function openServ() {
    services.style.display = services.style.display === 'flex' ? 'none' : 'flex';
    dropDw.style.transform = dropDw.style.transform === 'rotate(180deg)' ? 'rotate(0)' : 'rotate(180deg)'
  }
  window.addEventListener("click", function (event) {
    if (!serv.contains(event.target) && services.style.display === "flex") {
      services.style.display = "none";
      dropDw.style.transform = "rotate(0deg)";
    }
  })
}
